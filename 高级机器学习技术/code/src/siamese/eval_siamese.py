"""评估训练好的 Siamese Network。

脚本作用：
1. 加载 `best_siamese.pth`；
2. 读取测试集图像对；
3. 计算每个图像对的 embedding 距离；
4. 使用训练阶段保存的最佳阈值得到预测标签；
5. 保存 `test_scores.csv`、`metrics.json` 和 `roc_curve.png`。
"""
...
# 允许直接运行 `python src/siamese/eval_siamese.py`。
...
@torch.no_grad()
def predict_distances(model: SiameseNetwork, loader: DataLoader, device: torch.device) -> list[dict[str, object]]:
    """批量预测所有测试图像对的距离。"""
    model.eval()
...
def parse_args() -> argparse.Namespace:
    """解析测试脚本命令行参数。"""
    parser = argparse.ArgumentParser(description="Evaluate Siamese Network baseline.")
    ...
    parser.add_argument("--checkpoint", type=Path, default=Path("results/siamese/best_siamese.pth"))
...
def main() -> None:
    ...
    # 阈值必须来自验证集，测试集不能参与阈值选择。
    best_threshold = config.get("best_threshold", checkpoint.get("best_threshold"))
    if best_threshold is None:
        raise ValueError("best_threshold is missing. Please run train_siamese.py first.")
    ...
    save_roc_curve(
        ...
        title="Siamese Network ROC Curve",
    )
